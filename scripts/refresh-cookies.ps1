...
writes them to cookies.txt in the repo root (Netscape format, LF newlines, no BOM),
and uploads them as the INSTAGRAM_COOKIES GitHub Actions secret so the next
scheduled run of update-tracker-stats.yml can refresh Instagram view counts.
...
# Build the cookie jar text ourselves with a GUARANTEED Netscape magic header line as line 1
# and explicit LF newlines. Do NOT rely on the magic line surviving a comment-line filter:
# yt-dlp (and http.cookiejar) reject the file outright if the first line isn't this exact pattern,
# and a missing/misplaced header is exactly how the CI secret broke before.
$MAGIC = "# Netscape HTTP Cookie File"
$body = @($MAGIC, "# This file is generated by refresh-cookies.ps1. Do not edit.", "") + @($instagramOnly)
$content = ($body -join "`n") + "`n"

# Write with UTF-8 (no BOM) and LF endings so the file is byte-identical to what we push to GitHub.
$utf8NoBom = New-Object System.Text.UTF8Encoding $false
[System.IO.File]::WriteAllText($CookiesPath, $content, $utf8NoBom)
...
# Pipe the exact $content string (no BOM, LF newlines) straight to gh; PS 5.1's default
# $OutputEncoding is UTF-8-with-BOM, which would prepend a BOM and make yt-dlp reject the jar.
$prevOutputEncoding = $OutputEncoding
$OutputEncoding = $utf8NoBom
try {
    $content | & gh secret set $SecretName --repo $RepoSlug
...
Write-Host "Done. Next scheduled run of update-tracker-stats.yml will use the fresh cookies." -ForegroundColor Green
